...
- Pinned repo ref checked: `fb65f61d80344a9d768292c228d6cbb30f52c362` on `main`, checked by read-only remote HEAD and compare metadata on 2026-06-04.
...
Skill Scout read-only release-closeout follow-up reviewed upstream default-branch movement from `5032f7e75c3ea7ad02e33151486fb27b46a4ace5` to `620a061a90d100b57f958980e7bc37d533cd798b` using read-only remote HEAD and GitHub compare metadata only. The compare was 8 commits ahead and added several upstream agent-role files plus Codex conversion/install documentation and shell script changes. Because this source has no registered affected toolkit methods and the new Codex install/conversion material would be unsafe to copy or activate without separate review, the recommendation remains passive source-safety tracking only. This refresh updates source tracking only and does not approve source script execution, install flow use, Codex conversion use, raw agent prompt copying, new agent creation, roster import, method extraction, source copying, package or CI changes, product-repo changes, global config changes, or runtime activation.

Skill Scout read-only post-merge follow-up reviewed upstream default-branch movement from `620a061a90d100b57f958980e7bc37d533cd798b` to `fb65f61d80344a9d768292c228d6cbb30f52c362` using read-only remote HEAD and GitHub compare metadata only after live freshness changed during closeout. The compare was 12 commits ahead and added more upstream agent-role files plus README counter changes. No registered toolkit methods are affected. The source remains passive taxonomy evidence only; rapid roster growth reinforces the existing warning against roster import, prompt copying, new-agent creation, source copying, or runtime activation. This refresh updates source tracking only and does not approve method extraction, source script execution, install or conversion flow use, raw prompt copying, agent creation, package or CI changes, product-repo changes, global config changes, or runtime activation.
