...
| agents | `agents/*.md` | .codex/agents/*.toml records repo-local custom-agent file presence only; it is not spawn proof | .ai-toolkit/agents/*.md for packaged agent source material | Existing non-active agent specs remain top-level documented assets | manifest hashes plus TOML field validation; report TOML presence separately from compiled fallback and actual spawn proof |
