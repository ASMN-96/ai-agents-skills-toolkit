...
| GSD-style discipline | `active-if-detected` external discipline when already available | `owner-approved-install` for any install/vendor/global config | n/a | Do not vendor, duplicate, or claim GSD invocation without observed workflow output |
...
| GSD | Governance, Planning, Source of Truth | active-if-detected external-only | external Codex/plugin/operator environment | Phase/state execution discipline when already available | Vendoring into toolkit or project, or claiming invocation from metadata only | selected/not invoked/manual fallback or observed output | Yes for any install/config | External discipline tool |
