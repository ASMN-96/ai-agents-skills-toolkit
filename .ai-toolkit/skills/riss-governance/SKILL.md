...
description: Use when doing serious RISS work, or when explicitly invoked as an opt-in governance layer for repo governance, source-of-truth checks, task routing, branch/PR/CI/CodeRabbit discipline, backend/security caution, UI/runtime validation, or completion verification. Do not use as approval for writes, installs, migrations, broad plugin use, or global config changes.
---
