...
function deferredEvidence() {
  return {
    ...
    level3Closeout: {
      ...
      report: "docs/level3-closeout.md"
    },
...
async function withFixture(evidence, callback) {
  ...
  try {
    ...
    await writeFile(path.join(fixture, "docs", "level3-closeout.md"), "# Level 3 Closeout\n", "utf8");
...
test("reports deferred evidence without failing default audit mode", async () => {
  await withFixture(deferredEvidence(), async (fixture) => {
    const result = await runValidator(fixture);

    assert.equal(result.code, 0, result.stderr);
    assert.match(result.stdout, /BLOCKED validate-level4-readiness/);
    assert.match(result.stdout, /evidence status: deferred/);
    assert.match(result.stdout, /passed project pilots: 1\/3/);
    assert.match(result.stdout, /promotion decision is not owner-approved/);
  });
});

test("fails deferred evidence in require-ready mode", async () => {
  await withFixture(deferredEvidence(), async (fixture) => {
