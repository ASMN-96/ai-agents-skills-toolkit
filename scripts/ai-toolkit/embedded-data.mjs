export const TOOLKIT_VERSION = "0.2.3";
...
export const SOURCE_OF_TRUTH_MAP = [
  ...
  },
  {
    ...
    runtimeCopy: ".codex/agents/*.toml records repo-local custom-agent file presence only; it is not spawn proof",
    ...
    driftControl: "manifest hashes plus TOML field validation; report TOML presence separately from compiled fallback and actual spawn proof"
  },
  {
    domain: "compiled-agents",
    canonicalSource: "compiled-agents/*.compiled.md",
    runtimeCopy: "none; compiled agents are fallback documentation and inline-use material, not native spawn proof",
    ...
    driftControl: "manifest hashes plus source_commit/compiler/registry provenance"
  },
  {
    domain: "registries",
...
export const TOOL_ENTRIES = [
  ...
  ["gsd-core", "GSD Core", "open-gsd/gsd-core", "https://github.com/open-gsd/gsd-core", "phase-state-governance", "phase/state planning and execution discipline for governed multi-step work", "active-if-detected", "active-if-detected when project/operator-owned; owner-approved install when absent"],
  ["repomix", "Repomix", "yamadashy/repomix", "https://repomix.com", "context-packing", "scoped repository context packing and token counting when project-owned or owner-approved", "active-if-detected", "active-if-detected for project-owned config/dependency; owner-approved scoped packs and token counts when absent"],
