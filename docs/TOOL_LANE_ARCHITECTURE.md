...
- Resources/tools: AGENTS.md, README.md, docs/NO_FAKE_VALIDATION_POLICY.md, docs/REAL_PROJECT_READINESS.md, docs/PROJECT_TOOLING_OPERATING_MODEL.md, methods/governance/task-intake-routing-gate.md, GSD Core as first-class governed tool metadata, and Superpowers as an external-only discipline tool.
...
- GSD Core posture: `active-if-detected` when already available and `owner-approved-install` when absent; no install, vendoring, package change, CI, MCP, hooks, global config, runtime activation, or invocation claim without approval and observed workflow output.
- Install posture: metadata-only; no project install from toolkit registry presence.
