...
| nagdy-guard-skills | Nagdy Guard Skills | active-method | Do later | v0.2.3 `SYNCED_ADOPTED`; guard-pass concepts for code, tests, and docs routed through existing methods and skills | Keep code/test/docs guard-pass guidance concise and non-duplicative | No install, raw skill copying, new active guard skills, WordPress guard adoption, WooCommerce guard adoption, global activation, or product-repo changes |
...
| tweetclaw | TweetClaw | active-reference | Do later | Reviewed OpenClaw plugin and social-media skill release-evidence reference | Keep as reference-only input for skill packaging, approval gates, and source-safety comparisons | No install, activation, raw skill copying, credentials, live social-media actions, product-repo changes, or NVIDIA-verified/signed-status claims without observed evidence |
