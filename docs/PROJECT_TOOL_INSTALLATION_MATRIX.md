...
| code-review-graph | Architecture, Repo Intelligence, and Token Context | active-read-only | read-only source/reference | Manual/static context and source intelligence | Install, MCP/global config, product indexing/scanning, package change, CI wiring | manual/static notes or actual output only if separately approved and run | Yes for any install/indexing/scanning | No default execution or evidence claim |
