...
| GSD Core | `active-if-detected` first-class governed tool metadata when already available | `owner-approved-install` for any install/vendor/package/CI/MCP/hook/global config | n/a | Do not vendor, duplicate, or claim GSD invocation without observed workflow output |
...
| GSD Core | Governance, Planning, Source of Truth | active-if-detected | project/operator environment after detection or owner approval | Phase/state execution discipline when already available | Vendoring into toolkit or project, or claiming invocation from metadata only | selected/not invoked/manual fallback or observed GSD output | Yes for any install/config/package/CI/MCP/hook change | First-class governed tool metadata; not bundled runtime |
...
| Repomix | Architecture, Repo Intelligence, and Token Context | active-if-detected as metadata only; absent-but-approved -> owner-approved-install | target project only after scoped owner approval, even when project-owned/detected | Scoped context packs and token counts | Default install, automatic whole-repo dump, package change, CI wiring, MCP/global config, product scanning | approved scoped pack/token-count output only if actually run | Yes for install/execution/package/CI/MCP/global config | No default execution or whole-repo dump |
