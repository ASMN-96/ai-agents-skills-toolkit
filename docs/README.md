...
`docs/STALE_UNVERIFIED_DATA_CLASSIFICATION.md` is generated by `scripts/classify-stale-unverified-data.mjs` and classifies old version metadata, unknown review placeholders, project/private naming, local paths, non-HTTPS URLs, and unverified metadata without deleting anything.

`docs/ROLLOUT_MATURITY_AND_PUBLIC_RELEASE_READINESS.md` defines Levels 0-5, required validators, warning thresholds, owners, runtime policy, public release checklist, rollback, and promotion rules.
