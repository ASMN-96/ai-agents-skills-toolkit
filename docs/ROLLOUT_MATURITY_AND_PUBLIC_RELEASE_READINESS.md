...
- Enterprise external-tool metadata remains mostly `unknown-review-required`.
- Source/tool approval owners are not fully assigned.
...
- Public/private leak report still has unresolved release blockers.
- Generic public naming is not implemented as aliases/wrappers.
- Public package allowlist is designed but not enforced as a release command.
- `LICENSE`, `CONTRIBUTING`, and `CODE_OF_CONDUCT` are absent and require owner/legal/community decisions.
- `SECURITY.md` exists but is minimal and needs public-reporting, supported-version, disclosure, and response-time details before public release.
...
- The known compiled-agent version drift WARN remains allowed only because `docs/COMPILED_AGENT_COMPILE_CONTRACT.md` defines the future deterministic fix path.
...
- No private-overlay-only findings may exist in public package output.
...
- Stale/unverified classification has no `private-overlay-only` or unresolved `review-required` content in public package output.
