...
External support tools must be marked `external: true`. GSD Core must be represented as first-class governed tool metadata with a watched source record. Superpowers, Codex plugins, Playwright/browser, GitHub/gh, Supabase tooling/docs, approved design artifacts, CodeRabbit, and discovery sources remain external capabilities. Figma is excluded from v0.2 current-scope recommendations unless a separately approved design artifact is supplied outside the default model.
