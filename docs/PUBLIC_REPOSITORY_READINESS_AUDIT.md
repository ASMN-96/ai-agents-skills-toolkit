...
| `docs/PUBLIC_PRIVATE_LEAK_REPORT.md` | Leak scan is report-only and currently lists 221 findings across the whole tree, including 51 `move-to-private-overlay` findings. | blocker | Regenerated the scan and kept publication blocked instead of treating allowlist validation as whole-repo approval. | Review generated scan results and decide what must be rewritten, excluded, archived, or accepted before publication. |
